...
Icons[PlayerStates.Invisible] = {
    tooltip = tr('You are invisible'),
    path = '/images/game/states/invisible',
...
-- Exportar funções para o módulo
_G.game_topbar = {
    show = show,
    hide = function() if topBar then topBar:setVisible(false) end end,
    toggle = function() if topBar then topBar:setVisible(not topBar:isVisible()) end end,
    refresh = refresh,
    setupTopBar = setupTopBar
}

-- Também exportar para modules se necessário
if not modules.game_topbar then
    modules.game_topbar = _G.game_topbar
end
