...
function init()  
  ...
  battleButton = modules.client_topmenu.addLeftButton("battleButton", "Battle", "/data/images/topbuttons/battle", toggle)
...
function doCreatureFitFilters(creature)
  ...
  if not localPlayerPos or pos.z ~= localPlayerPos.z or not creature:canBeSeen() then return false end
  ...
  local hidePets = filterPanel.buttons.hidePets:isChecked()

  if hidePlayers and creature:isPlayer() then
    return false
  elseif hideNPCs and creature:isNpc() then
    return false
  elseif hideMonsters and creature:isMonster() and not creature:isNpcLike() then
    return false
  ...
  elseif hideParty and creature:getShield() > ShieldWhiteBlue then
    return false
  elseif hidePets and creature:isNpcLike() then
    return false
  end
...
function sortCreatures(creatures)
  ...
  if getSortType() == 'distance' then
    ...
    table.sort(creatures, function(a, b) 
      ...
      if getDistanceBetween(playerPos, aPos) == getDistanceBetween(playerPos, bPos) then
        return ages[a:getId()] > ages[b:getId()]
      end
      return getDistanceBetween(playerPos, aPos) > getDistanceBetween(playerPos, bPos) 
    end)
