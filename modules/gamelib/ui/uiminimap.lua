...
function UIMinimap:clearAllFlags()
    local i = 1
    while i <= #self.flags do
        local flag = self.flags[i]
        if flag.icon == 20 then
            flag:destroy()  -- onDestroy será chamado, removendo a flag de self.flags
        else
            i = i + 1  -- Incrementa somente se a flag não foi destruída
        end
    end
end
...
function UIMinimap:onStyleApply(styleName, styleNode)
  if not styleNode or not styleNode.children then return end
  for _, childNode in pairs(styleNode:children()) do
    local name = childNode:getTag()
    local value = childNode:getValue()
