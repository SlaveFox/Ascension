...
-- em algum lugar carregado cedo (ex.: modules/gamelib/protocolgame.lua)
connect(g_game, {
  onGameStart = function()
    extendedJSONData = {}   -- começa limpo
  end,
  onGameEnd = function()
    extendedJSONData = {}   -- se cair no meio de um pacote fragmentado, não reaproveita lixo
  end
})
...
function ProtocolGame:onExtendedOpcode(opcode, buffer)
  ...
  if callback then
    ...
    if status ~= "S" and status ~= "P" then
      -- ✅ Captura melhor os erros do JSON
      local success, json_data = pcall(function() return json.decode(extendedJSONData[opcode]) end)
      extendedJSONData[opcode] = nil
      if not success then
        print("[ERRO] Falha ao decodificar JSON no opcode " .. opcode .. ": " .. tostring(json_data))
        return
      end
