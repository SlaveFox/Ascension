...
ProtocolGame.registerExtendedOpcode(221, function(protocol, opcode, buffer)
  if buffer:sub(1, 10) == "playsound:" then
    local filename = buffer:sub(11)
    local fullPath = "/sounds/" .. filename
    if g_sounds and g_resources.fileExists(fullPath) then
      local ch = g_sounds.getChannel(2)
      ch:stop()
      ch:enqueue(fullPath, 1)
    else
      print("⚠️ Som não encontrado: " .. fullPath)
    end

  elseif buffer == "stopsound" then
    local ch = g_sounds.getChannel(2)
    if ch then ch:stop() end
  end
end)
