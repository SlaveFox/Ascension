...
function bindKeys()
  ...
  g_keyboard.bindKeyDown('Enter', function() modules.game_console.sendCurrentMessage() end, gameRootPanel)
  local lastAction = 0
...
function createThingMenu(menuPosition, lookThing, useThing, creatureThing)
  ...
  if creatureThing then
    ...
    menu:addSeparator()
      ...
      menu:addOption(tr('Jutsus'), function() modules.game_console.sendMessage("!tecnicas") end)
    end
	
    if creatureThing:isLocalPlayer() then
      menu:addOption(tr('Set Outfit'), function() g_game.requestOutfit() 
    end)
    if modules.game_bestiary then
        menu:addOption(tr('Open Bestiary'), function() modules.game_bestiary.toggle() end)
    end
    if activeloot == false then
      ...
      if creatureThing:getPosition().z == localPosition.z then
		if g_game.getAttackingCreature() ~= creatureThing then
			if not creatureThing:isNpcLike() then
      ...
      if creatureThing:isPlayer() then
        ...
        menu:addOption(tr('Inspect') .. ' ' .. creatureName, function() modules.game_inspect.inspect(creatureName) end)
	...
	if creatureThing:isNpcLike() then
    menu:addOption(tr('Set OutfitPet'), function() modules.game_petfit.requestImages(creatureThing:getName()) end)
	end
    menu:addSeparator()
