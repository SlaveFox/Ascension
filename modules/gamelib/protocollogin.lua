...
function ProtocolLogin:parseCharacterList(msg)
  ...
  if g_game.getProtocolVersion() > 1010 then
    ...
    for i=1,charactersCount do
      ...
      character.vocationName = msg:getString()
      if g_game.getFeature(GamePreviewState) then
  ...
  -- extras enviados pelo server (opcionais): email e recoveryKey
  if msg.getUnreadSize then
    if msg:getUnreadSize() > 0 then
      account.email = msg:getString()
    end
    if msg:getUnreadSize() > 0 then
      account.recoveryKey = msg:getString()
    end
  end
