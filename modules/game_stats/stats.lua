...
updateEvent = nil
lastUpdateTime = 0
UPDATE_THROTTLE = 1000 -- Reduzido de 500ms para 1000ms (1 segundo)
...
function init()
  ...
  -- Otimização: aumentar intervalo inicial de 200ms para 1000ms
...
function update()
  -- Otimização: usar throttling para evitar updates desnecessários
  local now = g_clock.millis()
  if (now - lastUpdateTime) < UPDATE_THROTTLE then
    updateEvent = scheduleEvent(update, UPDATE_THROTTLE)
    return
  end
  lastUpdateTime = now
