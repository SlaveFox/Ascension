-- @docclass
UIPopupScrollMenuCreate = extends(UIWidget, "UIPopupScrollMenuCreate")

local currentMenu

function UIPopupScrollMenuCreate.create()
  local menu = UIPopupScrollMenuCreate.internalCreate()
...
function UIPopupScrollMenuCreate:clearOptions()
    -- Remove todos os filhos do scrollArea
    local children = self.scrollArea:getChildren()
    for i = #children, 1, -1 do
        self.scrollArea:removeChild(children[i])
    end
end

function UIPopupScrollMenuCreate:setScrollbarStep(step)
  self.scrollBar:setStep(step)
end

function UIPopupScrollMenuCreate:display(pos)
  -- Limpa as opções existentes antes de adicionar novas
  self:clearOptions()

  -- Não exibe se nenhuma opção foi adicionada
...
function UIPopupScrollMenuCreate:onGeometryChange(oldRect, newRect)
...
function UIPopupScrollMenuCreate:addOption(optionName, optionCallback, shortcut)
    ...
    optionWidget.onClick = function(widget)
        if type(optionCallback) == "function" then
            ...
            print("Erro: optionCallback não é uma função.")
        end
    end
...
function UIPopupScrollMenuCreate:addSeparator()
  g_ui.createWidget(self:getStyleName() .. 'Separator', self.scrollArea)
end

function UIPopupScrollMenuCreate:onDestroy()
...
function UIPopupScrollMenuCreate:onMousePress(mousePos, mouseButton)
...
function UIPopupScrollMenuCreate:onKeyPress(keyCode, keyboardModifiers)
